...
class TSPIslandIndividual implements IslandIndividual{
  ...
  TSPIslandIndividual(int geneLength, Random rng,
    ...
    route = []
    route[0] = 1   // geneLength are subscripted from 1 upwards
    route[cities] = 1   // last city is always 1
    int place = rng.nextInt(cities) + 1   //1.. geneLength
    for ( i in 1 ..< cities ) {
...
//    println "Route: $route, $geneLength $fitness"
  ...
  TSPIslandIndividual(int cities,
    ...
    route = []
    route[0] = 1   // geneLength are subscripted from 1 upwards to geneLength-1
    route[cities] = 1
  ...
  void mutate(Random rng) {
    int place1 = rng.nextInt(cities - 1) + 1  //1..geneLength-1
